 ...
 * 演示短串分享功能，
 */
public class ShareDemoActivity extends AppCompatActivity implements OnGetPoiSearchResultListener,
        OnGetShareUrlResultListener, OnGetGeoCoderResultListener, BaiduMap.OnMarkerClickListener {
    ...
    public void onGetPoiDetailShareUrlResult(ShareUrlResult result) {
        ...
        intent.putExtra(Intent.EXTRA_TEXT, "您的朋友通过百度地图SDK与您分享一个POI点详情: " + currentAddr
                + " -- " + result.getUrl());
    ...
    public void onGetLocationShareUrlResult(ShareUrlResult result) {
        // 分享短串结果
        Intent intent = new Intent(Intent.ACTION_SEND);
        intent.putExtra(Intent.EXTRA_TEXT, "您的朋友通过百度地图SDK与您分享一个位置: " + currentAddr
                + " -- " + result.getUrl());
        intent.setType("text/plain");
        startActivity(Intent.createChooser(intent, "将短串分享到"));
    }
